...
class Contrastive_mod_linear_Model(nn.Module):
    def __init__(self, backbone, head='linear', features_dim=128):
        super(Contrastive_mod_linear_Model, self).__init__()
    ...
    def forward(self, x):
        ...
        # Reshape the backbone output to (batch_size, backbone_dim)
        features = self.backbone(x).squeeze().view(x.size(0), self.backbone_dim)
