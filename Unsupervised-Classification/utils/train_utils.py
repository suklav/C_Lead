...
def simclr_train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, batch in enumerate(train_loader):
        images = batch['image']
        images_augmented_1 = batch['image_augmented_1']
        images_augmented_2 = batch['image_augmented_2']

        b, c, h, w = images.size()
        input_ = torch.cat([images.unsqueeze(1), images_augmented_1.unsqueeze(1), images_augmented_2.unsqueeze(1)], dim=1)
        input_ = input_.view(-1, c, h, w) 
        input_ = input_.cuda(non_blocking=True)
        targets = batch['target'].cuda(non_blocking=True)

        
        output = model(input_).view(b, 3, -1)
        # print(output.size())
