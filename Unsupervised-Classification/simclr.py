...
def main():
    ...
    # from models.resnet34_own import ResNet34_class
    # model = ResNet34_class()
    # checkpoint = torch.load('./Results/Resnet34/backbone34_own.pth')
    # model.load_state_dict(checkpoint)
    # from models.models import Contrastive_mod_linear_Model
    # model = Contrastive_mod_linear_Model(model)

    # # For Resnet 50

    # from models.resnet50_test import ResNet
    # model = ResNet([3,4,6,3])
    # checkpoint = torch.load('./Results/Resnet50/backbone50.pth')
    # model.load_state_dict(checkpoint)
    # from models.models import Contrastive_mod_linear_Model
    # model = Contrastive_mod_linear_Model(model)
    ...
    from models.MyResnet import ResNetCifar10
    model = ResNetCifar10(num_classes=10)
    checkpoint = torch.load('./Results/Resnet18/backbone_own.pth')
    model.backbone.load_state_dict(checkpoint)
    from models.models import Contrastive_mod_linear_Model
    model = Contrastive_mod_linear_Model(model.backbone)
    ...
    for epoch in range(start_epoch, p['epochs']):
        ...
        torch.save(model.backbone.state_dict(), './Results/Resnet18/backboneckpt.pth')

    # Save final model
    torch.save(model.state_dict(), p['pretext_model'])
    torch.save(model.backbone.state_dict(), './Results/Resnet18/backbone_18_trained.pth')
